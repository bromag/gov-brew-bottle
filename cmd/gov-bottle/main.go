...
func main() {
	...
	if finalWorkdir == "" {
		fmt.Fprintln(os.Stderr, "error: missing workdir. Set --workdir or DEFAULT_WORKDIR in .env")
		os.Exit(2)
	}
	...
	if cliCfg.DryRun {
		...
		if err := os.MkdirAll(finalWorkdir, 0o755); err != nil {
			fmt.Fprintln(os.Stderr, "error: failed to create workdir:", err)
			os.Exit(1)
		}
		...
		if cliCfg.UploadJSON {
			if envCfg.NexusUser == "" || envCfg.NexusPass == "" {
				fmt.Fprintln(os.Stderr, "error: Nexus user, Nexus pass or Nexus password is empty")
				os.Exit(2)
			}
	...
	fmt.Printf("tag=%s\nworkdir=%s\nnexus=%s\n", finalTag, finalWorkdir, finalNexusBase)
	fmt.Fprintln(os.Stderr, "non-dry-run not implemented yet")
	os.Exit(3)
}
